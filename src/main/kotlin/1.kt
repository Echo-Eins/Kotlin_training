...
// БЛОК 7: Изолированные классы (Sealed Classes)

// В той же стереометрии результаты вычислений могут быть очень разными. Допустим, что мы ищем пересечение двух объектов.
// В таком случае результатом могут быть:
// 1. Одно единственное число (если это расстояние)
// 2. Вектор или точка (если пересеклись прямая или плоскость)
// 3. Прямая (если пересеклись две плоскости)
// 4. Ничего (если они параллельны)

// Как функции вернуть результат, если тип этого результата постоянно меняется?
// Писать класс Result, в котором будет 10 пустых переменных под каждый случай (и 9 из них будут null) - это уродство

// В Kotlin для этого придумали sealed class (изолированный класс). Это как коробка Шредингера, которая жестко ограничивает варианты того, что внутри
...
// ФАЗА 3 - Асинхронность
// Kotlin содержит в себе корутины (Coroutines)
// Корутины - это легковесные потоки. Их можно запускать хоть миллион сразу

// Главная идея: Корутина может останавливать свою работу (Suspend), освобождая рабочий поток для других корутин,
// а потом возвращаться и продолжать с того же места

// Базовый арсенал Асинхронности:
/*
1) runBlocking {...} (Блокирующий мост)
Обычные функции Kotlin не умеют работать с асинхронностью напрямую. Им нужен мост. runBlocking создает изолированный
мир корутин и БЛОКИРУЕТ основной поток до тех пор, пока все корутины внутри него не завершат работу.
В продакшене его используют редко, но для функции main() и тестов - это точка входа

2) launch {...} (Выстрели и забыл)
Это строитель корутин. Он мгновенно создает фоновую задачу и летит дальше по коду, не дожидаясь ее выполнения.

3) delay(timeMills: Long) (Умная пауза)
Главная функция асинхронного мира. В отличие от Thread.sleep(), которая намертво замораживает весь поток процессора,
delay просто говорит: "Я корутина, ухожу спать на 2 секунды. Отдайте этот поток кому-нибудь другому, пока я не проснусь"

// Под капотом:
Корутина - это не поток. Это просто кусок кода, объект в куче (Heap), который весит пару десятков байт

Операционная система вообще не знает об их существовании. Ядро ОС видит только классические системные потоки (условный пул из 4х потоков)
А уже внутри этих 4х потоков виртуальная машина Kotlin сама жонглирует сотнями тысяч корутин

Для этого используется механизм suspension - приостановка.
Когда обычная функция вызывает что-то долгое (например, ждет ответ от базы данных), она намертво блокирует системный поток. Поток стоит и ничего не делает, пока не придут данные.
Когда корутина натыкается на долгое ожидание (например, функцию delay()), она делает магию:

Она говорит потоку: "Я пошла спать, вот тебе закладка на место, где я остановилась. Иди работай дальше".

Корутина "открепляется" от системного потока (Suspend) и ложится в память.

Освободившийся системный поток мгновенно берет в работу другую корутину.

Когда данные пришли (или таймер delay истек), первая корутина просыпается, встает в очередь и ждет, когда любой свободный поток подхватит её с места "закладки" и доделает работу.
 */

// Анатомия магии: State Machine (Конечный автомат)
// Как компилятор Kotlin делает эти "закладки"?
// Если написать перед функцией магическое слово suspend (приостанавливаемая), компилятор под капотом разорвет твою
// линейную функцию в клочья и превратит в конечный автомат (State Machine)
